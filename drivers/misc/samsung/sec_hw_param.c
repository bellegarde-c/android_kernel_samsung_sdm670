		...
		struct device_attribute *attr, char *buf)
{
	...
	sysfs_scnprintf(buf, info_size, "\"PON\":\"%llx\",",
			         ...
			         struct device_attribute *attr, char *buf)
{
	...
	sysfs_scnprintf(buf, info_size, "\"REV1\":\"%02x\",", get_ddr_revision_id_1());
	sysfs_scnprintf(buf, info_size, "\"REV2\":\"%02x\",", get_ddr_revision_id_2());
	sysfs_scnprintf(buf, info_size, "\"SIZE\":\"%02x\",", get_ddr_total_density());
...
static ssize_t show_extra(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t offset = 0;
	unsigned int reset_reason;
...
static DEVICE_ATTR(extra, 0440, show_extra, NULL);

#ifdef CONFIG_SEC_PM
static ssize_t show_pwrsrc(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sec_get_pwrsrc(buf);
}
...
static int __init sec_hw_param_init(void)
{
	...
	sec_reset_reason_dev = sec_device_create(0, NULL, "sec_reset_reason");
	if (IS_ERR(sec_reset_reason_dev)) {
		pr_err("Failed to create devce\n");
		return -ENODEV;
	}

	err_extra = sysfs_create_group(&sec_reset_reason_dev->kobj,
			&sec_reset_reason_attribute_group);
	if (unlikely(err_extra)) {
		pr_err("Failed to create device files!\n");
		sec_device_destroy(sec_reset_reason_dev->devt);
	}

	return (err_hw_param | err_extra);
}
