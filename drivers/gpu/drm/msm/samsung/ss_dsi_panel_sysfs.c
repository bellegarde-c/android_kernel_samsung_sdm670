		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	struct candela_map_table *normal_table, *hbm_table;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%x %x %x %x %x %x", &val1, &val2, &val3, &val4, &val5, &val6);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* len val0 val1 ~ val9 */
	sscanf(buf, "%d %x %x %x %x %x %x %x %x %x %x",
			&txlen,
			&txval[0], &txval[1], &txval[2], &txval[3],
			&txval[4], &txval[5], &txval[6], &txval[7],
			&txval[8], &txval[9]);

	if (txlen == 0)
		goto err;

	LCD_INFO("txlen: %d, val: %x, %x, ...\n", txlen, txval[0], txval[1]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if( !ret) {
		ret = snprintf((char *)buf, 6, "0x%02x\n", ccd[0]);
	} else {
...
static DEVICE_ATTR(write_mtp, S_IRUGO | S_IWUSR | S_IWGRP, NULL, ss_write_mtp_store);
