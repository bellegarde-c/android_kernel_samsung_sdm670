		...
		enum dsi_cmd_set_type type, u8 *buffer, int level_key)
{
	...
	temp_buffer = kmalloc(set->cmds[0].msg.rx_len, GFP_KERNEL);
	if (temp_buffer == NULL) {
		LCD_ERR("fail to alloc temp_buffer..\n");
		return 0;
	}
...
static void ss_panel_parse_dt(struct samsung_display_driver_data *vdd)
{
	...
	LCD_DEBUG("anapass ddi power seq. flag: %d\n",
			vdd->dtsi_data.samsung_anapass_power_seq );
...
void ss_write_mtp(struct samsung_display_driver_data *vdd, u32 txlen, u32 *txval)
{
	...
	for (i = 0; i < txlen; i++)
		tx_cmds->cmds[0].msg.tx_buf[i] = txval[i];
	tx_cmds->cmds[0].msg.tx_len =  txlen;
...
void ss_panel_init(struct dsi_panel *panel)
{
	...
	if (vdd->mdnie.support_mdnie || vdd->support_cabc) {
		if (ss_panel_attached(ndx))
