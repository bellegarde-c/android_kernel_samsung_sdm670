...
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4, 9, 0) && LINUX_VERSION_CODE <= KERNEL_VERSION(4, 9, 999)
...
#define LCD_INFO(X, ...) pr_debug("%s %s : "X, LOG_KEYWORD, __func__, ## __VA_ARGS__)
#define LCD_INFO_ONCE(X, ...) pr_debug_once("%s %s : "X, LOG_KEYWORD, __func__, ## __VA_ARGS__)
#define LCD_ERR(X, ...) pr_debug("%s %s : "X, LOG_KEYWORD, __func__, ## __VA_ARGS__)
...
//	struct mutex vdd_ss_direct_cmdlist_lock;
	...
	*/
	...
	/*
	 * transmit display on cmd or not
	 */
...
void ss_write_mtp(struct samsung_display_driver_data *vdd, u32 txlen, u32 *txval);
