...
static int dsi_display_cmd_engine_enable(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to enable cmd engine, rc=%d\n",
		       display->name, rc);
		goto done;
	}
...
static void dsi_display_aspace_cb_locked(void *cb_data, bool is_detach)
{
	...
	if (!cb_data) {
		pr_debug("aspace cb called with invalid cb_data\n");
		return;
	}
	...
	if (is_detach) {
		...
		if (rc) {
			pr_debug("failed to get the iova rc %d\n", rc);
			goto end;
		}
		...
		if (IS_ERR_OR_NULL(display->vaddr)) {
			pr_debug("failed to get va rc %d\n", rc);
			goto end;
		}
	}
...
static void dsi_display_register_te_irq(struct dsi_display *display)
{
	...
	pdev = display->pdev;
	if (!pdev) {
		pr_debug("invalid platform device\n");
		return;
	}
...
#if defined(CONFIG_DISPLAY_SAMSUNG)
	/* ss_check_inval_te_period_init() registers already TE gpio irq
	 * to monitor invalid TE period.
	...
	if (display->panel->esd_config.status_mode != ESD_MODE_PANEL_TE) {
		pr_debug("%s: status_mode=%d, not ESD_MODE_PANEL_TE, skip!\n",
				__func__, display->panel->esd_config.status_mode);
		return;
	}
#endif
	...
	rc = devm_request_irq(dev, gpio_to_irq(display->disp_te_gpio),
			...
			"TE_GPIO", display);
	if (rc) {
		pr_debug("TE request_irq failed for ESD rc:%d\n", rc);
		goto error;
	}
...
static int dsi_host_alloc_cmd_tx_buffer(struct dsi_display *display)
{
	...
	if ((display->tx_cmd_buf) == NULL) {
		pr_debug("Failed to allocate cmd tx buf memory\n");
		rc = -ENOMEM;
		goto error;
	}
	...
	if (rc) {
		pr_debug("failed to register callback %d", rc);
		goto free_gem;
	}
...
static void dsi_display_parse_te_gpio(struct dsi_display *display)
{
	...
	pdev = display->pdev;
	if (!pdev) {
		pr_debug("Inavlid platform device\n");
		return;
	}
		...
		struct dsi_cmd_desc *cmd, u8 *payload, u32 payload_len)
{
	...
	if (cmd->msg.tx_len > payload_len) {
		pr_debug("Incorrect payload length tx_len %zu, payload_len %d\n",
		...
		u32 cmd_buf_len)
{
	...
	if (rc) {
		pr_debug("[DSI] command prepare failed. rc %d\n", rc);
		return rc;
	}
	...
	if (rc || !state) {
		pr_debug("[DSI] Invalid host state %d rc %d\n",
				state, rc);
		rc = -EPERM;
		goto end;
	}
					     ...
					     bool enable)
{
	...
	pr_debug("Force Clock HS Enable:%d\n", enable);
...
int dsi_display_soft_reset(void *display)
{
	...
	for (i = 0 ; i < dsi_display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to soft reset host_%d, rc=%d\n",
...
enum dsi_pixel_format dsi_display_get_dst_format(void *display)
{
	...
	if (!dsi_display || !dsi_display->panel) {
		pr_debug("Invalid params(s) dsi_display %pK, panel %pK\n",
			dsi_display,
		...
		int power_mode, void *disp)
{
	...
	if (!display || !display->panel) {
		pr_debug("invalid display/panel\n");
		return -EINVAL;
	}
				  ...
				  loff_t *ppos)
{
	...
	if (display->esd_trigger) {
		...
		if (rc) {
			pr_debug("Failed to trigger ESD attack\n");
			goto error;
		}
	}
				  ...
				  loff_t *ppos)
{
	...
	if (!esd_config) {
		pr_debug("Invalid panel esd config\n");
		rc = -EINVAL;
		goto error;
	}
	...
	if (!strcmp(buf, "reg_read\n")) {
		rc = dsi_panel_parse_esd_reg_read_configs(display->panel,
						display->panel_of);
		if (rc) {
			pr_debug("failed to alter esd check mode,rc=%d\n",
						rc);
			rc = user_len;
			goto error;
		}
...
static int dsi_display_debugfs_init(struct dsi_display *display)
{
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		...
		pr_debug("[%s] debugfs create dump info file failed, rc=%d\n",
		       display->name, rc);
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		rc = PTR_ERR(dump_file);
		pr_debug("[%s] debugfs for esd trigger file failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		rc = PTR_ERR(dump_file);
		pr_debug("[%s] debugfs for esd check mode failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(misr_data)) {
		...
		pr_debug("[%s] debugfs create misr datafile failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (IS_ERR_OR_NULL(dump_file)) {
			rc = PTR_ERR(dump_file);
			pr_debug("[%s] debugfs create %s failed, rc=%d\n",
	...
	if (!debugfs_create_bool("ulps_enable", 0600, dir,
		...
		pr_debug("[%s] debugfs create ulps enable file failed\n",
		       display->name);
		goto error_remove_dir;
	}
	...
	if (!debugfs_create_bool("ulps_suspend_enable", 0600, dir,
		...
		pr_debug("[%s] debugfs create ulps-suspend enable file failed\n",
		       display->name);
		goto error_remove_dir;
	}
...
static int dsi_display_set_ulps(struct dsi_display *display, bool enable)
{
	...
	if (rc) {
		pr_debug("Ulps controller state change(%d) failed\n", enable);
		return rc;
	}
	...
	if (rc) {
		pr_debug("Ulps PHY state change(%d) failed\n", enable);
		return rc;
	}
...
static int dsi_display_set_clamp(struct dsi_display *display, bool enable)
{
	...
	if (rc) {
		pr_debug("DSI ctrl clamp state change(%d) failed\n", enable);
		return rc;
	}

	rc = dsi_phy_set_clamp_state(m_ctrl->phy, enable);
	if (rc) {
		pr_debug("DSI phy clamp state change(%d) failed\n", enable);
		return rc;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("DSI Clamp state change(%d) failed\n", enable);
			return rc;
		}
...
static int dsi_display_ctrl_setup(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("DSI controller setup failed\n");
		return rc;
	}
...
static int dsi_display_ctrl_power_off(struct dsi_display *display)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		rc = dsi_ctrl_set_power_state(ctrl->ctrl,
			DSI_CTRL_POWER_VREG_OFF);
		if (rc) {
			pr_debug("[%s] Failed to power off, rc=%d\n",
			       ctrl->ctrl->name, rc);
			goto error;
		}
	}
					...
					unsigned int display_type)
{
	...
	if (display_type >= MAX_DSI_ACTIVE_DISPLAY) {
		pr_debug("display_type=%d not supported\n", display_type);
		return;
	}
	...
	if (kstrtol(str + strlen(":config"), INT_BASE_10,
				(unsigned long *)&value)) {
		pr_debug("invalid config index override: %s\n", boot_str);
		return;
	}
	...
	if (kstrtol(str + strlen(":timing"), INT_BASE_10,
				(unsigned long *)&value)) {
		pr_debug("invalid timing index override: %s. resetting both timing and config\n",
			boot_str);
...
static bool validate_dsi_display_selection(void)
{
	...
	for (i = 0; i < MAX_DSI_ACTIVE_DISPLAY; i++) {
		...
		for (j = 0; j < ctrl_count; j++) {
			...
			if (rc) {
				pr_debug("cell index not set for ctrl_nodes\n");
				return false;
			}
			if (ctrl_flags[index])
				return false;
		...
		for (j = 0; j < phy_count; j++) {
			...
			if (rc) {
				pr_debug("cell index not set phy_nodes\n");
				return false;
			}
			if (phy_flags[index])
				return false;
...
static int dsi_display_set_clk_src(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to set source clocks for master, rc=%d\n",
			   display->name, rc);
		return rc;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		rc = dsi_ctrl_set_clock_source(ctrl->ctrl,
			   &display->clock_info.mux_clks);
		if (rc) {
			pr_debug("[%s] failed to set source clocks, rc=%d\n",
				   display->name, rc);
			return rc;
		}
	}
		...
		bool enable)
{
	...
	for (i = 0 ; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to %s phy reset, rc=%d\n",
...
static int dsi_display_ctrl_update(struct dsi_display *display)
{
	...
	for (i = 0 ; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to update host_%d, rc=%d\n",
				   display->name, i, rc);
...
static int dsi_display_ctrl_init(struct dsi_display *display)
{
	...
	if (!display->panel->ulps_suspend_enabled || !display->ulps_enabled) {
		for (i = 0 ; i < display->ctrl_count; i++) {
			...
			if (rc) {
				pr_debug("[%s] failed to init host_%d, rc=%d\n",
				       display->name, i, rc);
				goto error_host_deinit;
			}
		}
...
static int dsi_display_ctrl_deinit(struct dsi_display *display)
{
	...
	for (i = 0 ; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to deinit host_%d, rc=%d\n",
			       display->name, i, rc);
		}
	}
...
static int dsi_display_ctrl_host_enable(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to enable host engine, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		rc = dsi_ctrl_set_host_engine_state(ctrl->ctrl,
						    DSI_CTRL_ENGINE_ON);
		if (rc) {
			pr_debug("[%s] failed to enable sl host engine, rc=%d\n",
			       display->name, rc);
			goto error_disable_master;
		}
	}
...
static int dsi_display_ctrl_host_disable(struct dsi_display *display)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		rc = dsi_ctrl_set_host_engine_state(ctrl->ctrl,
						    DSI_CTRL_ENGINE_OFF);
		if (rc)
			pr_debug("[%s] failed to disable host engine, rc=%d\n",
			       display->name, rc);
	}
...
static int dsi_display_vid_engine_enable(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to enable vid engine, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_vid_engine_disable(struct dsi_display *display)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		rc = dsi_ctrl_set_vid_engine_state(ctrl->ctrl,
						   DSI_CTRL_ENGINE_OFF);
		if (rc)
			pr_debug("[%s] failed to disable vid engine, rc=%d\n",
			       display->name, rc);
	}

	rc = dsi_ctrl_set_vid_engine_state(m_ctrl->ctrl, DSI_CTRL_ENGINE_OFF);
	if (rc)
		pr_debug("[%s] failed to disable mvid engine, rc=%d\n",
		       display->name, rc);
...
static int dsi_display_phy_enable(struct dsi_display *display)
{
	...
	rc = dsi_phy_enable(m_ctrl->phy,
			    ...
			    display->is_cont_splash_enabled);
	if (rc) {
		pr_debug("[%s] failed to enable DSI PHY, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_phy_disable(struct dsi_display *display)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc)
			pr_debug("[%s] failed to disable DSI PHY, rc=%d\n",
			       display->name, rc);
	}
				     ...
				     const struct mipi_dsi_msg *msg)
{
	...
	if (rc) {
		pr_debug("[%s] cmd transfer failed on master,rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] cmd transfer failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug("[%s] cmd trigger failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
	...
	if (rc) {
		pr_debug("[%s] cmd trigger failed for master, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_phy_sw_reset(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to reset phy, rc=%d\n", display->name, rc);
		goto error;
	}
	...
	struct mipi_dsi_device *dsi)
{
	...
	if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO) {
		...
		pr_debug("command mode not supported by ext bridge\n");
		return -ENOTSUPP;
	}
				 ...
				 const struct mipi_dsi_msg *msg)
{
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_ON);
	if (rc) {
		pr_debug("[%s] failed to enable all DSI clocks, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug("[%s] failed to wake up display, rc=%d\n",
		       display->name, rc);
	...
	if (display->ctrl_count > 1 && !(msg->flags & MIPI_DSI_MSG_UNICAST)) {
		...
		if (rc) {
			pr_debug("[%s] cmd broadcast failed, rc=%d\n",
			       display->name, rc);
			goto error_disable_cmd_engine;
		}
	} else {
		...
		if (((flags & DSI_CTRL_CMD_READ) && rc <= 0) ||
			...
			pr_debug("[%s] cmd transfer failed, rc=%d, flags=%x cmd = %x\n",
...
error_disable_cmd_engine:
	...
	if (ret) {
		pr_debug("[%s]failed to disable DSI cmd engine, rc=%d\n",
				display->name, ret);
	}
error_disable_clks:
	ret = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_OFF);
	if (ret) {
		pr_debug("[%s] failed to disable all DSI clocks, rc=%d\n",
		       display->name, ret);
	}
error:
	return rc;
...
static int dsi_display_mipi_host_init(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to register mipi dsi host, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_clocks_init(struct dsi_display *display)
{
	...
	if (IS_ERR_OR_NULL(src->byte_clk)) {
		...
		src->byte_clk = NULL;
		pr_debug("failed to get src_byte_clk, rc=%d\n", rc);
		/*
	...
	if (IS_ERR_OR_NULL(src->pixel_clk)) {
		...
		src->pixel_clk = NULL;
		pr_debug("failed to get src_pixel_clk, rc=%d\n", rc);
		/*
		 * Skip getting rest of clocks since one failed. This is a
		 * non-critical failure since these clocks are requied only for
		 * dynamic refresh use cases.
	...
	if (IS_ERR_OR_NULL(shadow->byte_clk)) {
		...
		shadow->byte_clk = NULL;
		pr_debug("failed to get shadow_byte_clk, rc=%d\n", rc);
		/*
		 * Skip getting rest of clocks since one failed. This is a
		 * non-critical failure since these clocks are requied only for
		 * dynamic refresh use cases.
	...
	if (IS_ERR_OR_NULL(shadow->pixel_clk)) {
		...
		shadow->pixel_clk = NULL;
		pr_debug("failed to get shadow_pixel_clk, rc=%d\n", rc);
		/*
		 * Skip getting rest of clocks since one failed. This is a
		 * non-critical failure since these clocks are requied only for
		 * dynamic refresh use cases.
	...
	struct dsi_clk_ctrl_info clk_state_info)
{
	...
	if (clk_state_info.client == DSI_CLK_REQ_MDP_CLIENT) {
		...
		pr_debug("invalid clk handle, return error\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug("[%s] failed to %d DSI %d clocks, rc=%d\n",
			   ...
			   enum dsi_clk_state new_state)
{
	...
	if ((clk & DSI_LINK_CLK) && (new_state == DSI_CLK_OFF) &&
		...
		if (rc)
			pr_debug("%s: failed enable ulps, rc = %d\n",
			       __func__, rc);
	}
	...
	if ((clk & DSI_CORE_CLK) && (new_state == DSI_CLK_OFF)) {
		...
		if (dsi_panel_initialized(display->panel) ||
			...
			if (rc)
				pr_debug("%s: Failed to enable dsi clamps. rc=%d\n",
					__func__, rc);
			...
			if (rc)
				pr_debug("%s: failed to disable ulps. rc=%d\n",
					__func__, rc);
		}
			   ...
			   enum dsi_clk_state curr_state)
{
	...
	if ((clk & DSI_LINK_CLK) && (l_type & DSI_LINK_LP_CLK)) {
		...
		if (display->ulps_enabled && mmss_clamp) {
			...
			rc = dsi_display_set_ulps(display, true);
			if (rc) {
				pr_debug("%s: Failed to enter ULPS. rc=%d\n",
					__func__, rc);
				goto error;
			}
		}
		...
		if (rc) {
			pr_debug("%s: Failed to disable dsi clamps. rc=%d\n",
				__func__, rc);
			goto error;
		}
	}
			    ...
			    enum dsi_clk_state curr_state)
{
	...
	if ((clk_type & DSI_CORE_CLK) &&
		...
		if (rc)
			pr_debug("[%s] failed to power off PHY, rc=%d\n",
				   display->name, rc);

		rc = dsi_display_ctrl_power_off(display);
		if (rc)
			pr_debug("[%s] failed to power DSI vregs, rc=%d\n",
				   display->name, rc);
	}
	return rc;
}
			  ...
			  enum dsi_clk_state new_state)
{
	...
	if ((clk_type & DSI_CORE_CLK) && (new_state == DSI_CLK_ON)) {
	  ...
	  pr_debug("updating power states for ctrl and phy\n");
		...
		if (rc) {
			pr_debug("[%s] failed to power on dsi controllers, rc=%d\n",
				   display->name, rc);
			return rc;
		}
		...
		if (rc) {
			pr_debug("[%s] failed to power on dsi phy, rc = %d\n",
				   display->name, rc);
			return rc;
		}
...
static int dsi_display_parse_dt(struct dsi_display *display)
{
	...
	for (i = 0; i < MAX_DSI_CTRLS_PER_DISPLAY; i++) {
		...
		if (!of_node) {
			if (!i) {
				pr_debug("No controllers present\n");
				return -ENODEV;
			}
			break;
		}
	...
	for (i = 0; i < MAX_DSI_CTRLS_PER_DISPLAY; i++) {
		...
		if (!of_node) {
			if (!i) {
				pr_debug("No PHY devices present\n");
				rc = -ENODEV;
				goto error;
			}
			break;
	...
	if (phy_count != display->ctrl_count) {
		pr_debug("Number of controllers does not match PHYs\n");
		rc = -ENODEV;
		goto error;
	}
	...
	if (!of_node) {
		...
		if (!endpoint) {
			pr_debug("no endpoint for ext bridge\n");
			rc = -ENODEV;
			goto error;
		}
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (IS_ERR_OR_NULL(ctrl->ctrl)) {
			...
			pr_debug("failed to get dsi controller, rc=%d\n", rc);
			ctrl->ctrl = NULL;
			goto error_ctrl_put;
		}
		...
		if (IS_ERR_OR_NULL(ctrl->phy)) {
			rc = PTR_ERR(ctrl->phy);
			pr_debug("failed to get phy controller, rc=%d\n", rc);
	...
	if (IS_ERR_OR_NULL(display->panel)) {
		...
		pr_debug("failed to get panel, rc=%d\n", rc);
		display->panel = NULL;
		goto error_ctrl_put;
	}
	...
	if (rc) {
		pr_debug("Lane map not found, rc=%d\n", rc);
		goto error_ctrl_put;
	}

	rc = dsi_display_clocks_init(display);
	if (rc) {
		pr_debug("Failed to parse clock data, rc=%d\n", rc);
		goto error_ctrl_put;
	}
...
static int dsi_display_res_deinit(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug("clocks deinit failed, rc=%d\n", rc);
					  ...
					  u32 bit_clk_rate)
{
	...
	if (bit_clk_rate == 0) {
		pr_debug("Invalid bit clock rate\n");
		return -EINVAL;
	}
					...
					struct link_clk_freq *bkp_freq)
{
	...
	rc = dsi_clk_update_parent(&display->clock_info.shadow_clks,
			      &display->clock_info.mux_clks);
	if (rc) {
		pr_debug("failed update mux parent to shadow\n");
		goto exit;
	}
	...
	for (i = 0; (i < display->ctrl_count) &&
		...
		if (rc) {
			pr_debug("wait4dynamic refresh failed for dsi:%d\n", i);
			goto recover_pix_clk;
		} else {
	...
	rc = dsi_clk_update_parent(&display->clock_info.src_clks,
			      &display->clock_info.mux_clks);
	if (rc)
		pr_debug("could not switch back to src clks %d\n", rc);
					  ...
					  struct dsi_display_mode *mode)
{
	...
	if (rc) {
		pr_debug("failed set link frequencies %d\n", rc);
		goto exit;
	}
				   ...
				   struct dsi_display_mode *dsi_mode)
{
	...
	if (!dfps_caps.dfps_support) {
		pr_debug("dfps not supported\n");
		return -ENOTSUPP;
	}

	if (dfps_caps.type == DSI_DFPS_IMMEDIATE_CLK) {
		pr_debug("dfps clock method not supported\n");
		return -ENOTSUPP;
	}
	...
	if (rc) {
		pr_debug("[%s] failed to dfps update host_%d, rc=%d\n",
				display->name, i, rc);
		goto error;
	}
		...
		u32 *b_fp_out)
{
	...
	if (!a_total || !new_fps) {
		pr_debug("Invalid pixel total or new fps in mode request\n");
		return -EINVAL;
	}
	...
	if (b_fp_new < 0) {
		pr_debug("Invalid new_hfp calcluated%d\n", b_fp_new);
		return -EINVAL;
	}
				...
				u32 curr_refresh_rate)
{
	...
	dsi_panel_get_dfps_caps(display->panel, &dfps_caps);
	if (!dfps_caps.dfps_support) {
		pr_debug("dfps not supported by panel\n");
		return -EINVAL;
	}
	...
	if (!curr_refresh_rate) {
		if (!dsi_display_is_seamless_dfps_possible(display,
			...
			pr_debug("seamless dynamic fps not supported for mode\n");
			return -EINVAL;
		}
		if (display->panel->cur_mode) {
			...
			pr_debug("cur_mode is not initialized\n");
			return -EINVAL;
		}
	}
	...
	default:
		pr_debug("Unsupported DFPS mode %d\n", dfps_caps.type);
		rc = -ENOTSUPP;
	}
				    ...
				    u32 flags)
{
	...
	if (!dsi_display_has_ext_bridge(display) && !priv_info) {
		pr_debug("[%s] failed to get private info of the display mode",
				display->name);
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug("[%s] failed to get host config for mode, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	if (mode->dsi_mode_flags &
		...
		if (rc) {
			pr_debug("[%s]DSI dfps update failed, rc=%d\n",
					display->name, rc);
			goto error;
		}
		...
		if (rc)
			pr_debug("dynamic clk change failed %d\n", rc);
		/*
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to update ctrl config, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
	...
	if (priv_info && priv_info->phy_timing_len) {
		for (i = 0; i < display->ctrl_count; i++) {
			...
			if (rc)
				pr_debug("failed to add DSI PHY timing params");
		}
	}
error:
	return rc;
...
static int _dsi_display_dev_init(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug("[%s] failed to parse dt, rc=%d\n", display->name, rc);
		goto error;
	}

	rc = dsi_display_res_init(display);
	if (rc) {
		pr_debug("[%s] failed to initialize resources, rc=%d\n",
		       display->name, rc);
		goto error;
	}
error:
...
static int _dsi_display_dev_deinit(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug("[%s] failed to deinitialize resource, rc=%d\n",
		       display->name, rc);
...
int dsi_display_cont_splash_config(void *dsi_display)
{
	...
	if (rc) {
		pr_debug("failed to vote gdsc for continuous splash, rc=%d\n",
							rc);
		mutex_unlock(&display->display_lock);
		return -EINVAL;
	}
	...
	if (!display->is_cont_splash_enabled) {
		pr_debug("Continuous splash is not enabled\n");
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_ON);
	if (rc) {
		pr_debug("[%s] failed to enable DSI link clocks, rc=%d\n",
		       display->name, rc);
...
int dsi_display_splash_res_cleanup(struct  dsi_display *display)
{
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_OFF);
	if (rc)
		pr_debug("[%s] failed to disable DSI link clocks, rc=%d\n",
		       display->name, rc);

	rc = sde_power_resource_enable(display->phandle,
			display->cont_splash_client, false);
	if (rc)
		pr_debug("failed to remove vote on gdsc for continuous splash, rc=%d\n",
				rc);
...
int dsi_display_clk_ctrl(void *handle, u32 clk_type, u32 clk_state)
{
	...
	if (rc)
		pr_debug("%s: failed set clk state, rc = %d\n", __func__, rc);
	mutex_unlock(&dsi_display_clk_mutex);
...
static int dsi_display_force_update_dsi_clk(struct dsi_display *display)
{
	...
	if (!rc) {
		...
		pr_debug("Failed to configure dsi bit clock '%d'. rc = %d\n",
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (display->panel->panel_mode != DSI_OP_CMD_MODE) {
		pr_debug("only supported for command mode\n");
		return -ENOTSUPP;
	}

	if (clk_rate <= 0) {
		pr_debug("%s: bitrate should be greater than 0\n", __func__);
		return -EINVAL;
	}
	...
	if (!rc) {
		...
		pr_debug("%s: Failed to prepare to configure '%d'. rc = %d\n",
		...
		void *data)
{
	...
	if (!drm || !display) {
		pr_debug("invalid param(s), drm %pK, display %pK\n",
				drm, display);
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug("[%s] debugfs init failed, rc=%d\n", display->name, rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug("[%s] sysfs init failed, rc=%d\n", display->name, rc);
		goto error;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to initialize ctrl[%d], rc=%d\n",
			       display->name, i, rc);
		...
		if (rc) {
			pr_debug("[%s] Failed to initialize phy[%d], rc=%d\n",
				display->name, i, rc);
		...
		if (IS_ERR_OR_NULL(info.c_clks[i].dsi_core_client)) {
			pr_debug("[%s] client creation failed for ctrl[%d]",
					dsi_client_name, i);
			goto error_ctrl_deinit;
		}
	}
	...
	if (IS_ERR_OR_NULL(display->clk_mngr)) {
		...
		pr_debug("dsi clock registration failed, rc = %d\n", rc);
		goto error_ctrl_deinit;
	}
	...
	if (IS_ERR_OR_NULL(handle)) {
		rc = PTR_ERR(handle);
		pr_debug("failed to register %s client, rc = %d\n",
		       client1, rc);
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to register ctrl clk_cb[%d], rc=%d\n",
			       display->name, i, rc);
			goto error_ctrl_deinit;
		}
		...
		if (rc) {
			pr_debug("[%s] failed to register phy clk_cb[%d], rc=%d\n",
			       display->name, i, rc);
			goto error_ctrl_deinit;
		}
	}
	...
	if (rc) {
		pr_debug("[%s] failed to initialize mipi host, rc=%d\n",
		       display->name, rc);
		goto error_ctrl_deinit;
	}
	...
	if (rc) {
		if (rc != -EPROBE_DEFER)
			pr_debug("[%s] failed to initialize panel driver, rc=%d\n",
			       display->name, rc);
		goto error_host_deinit;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to set phy clk freq, rc=%d\n",
					display->name, rc);
			goto error;
		}
	}
		...
		struct device *master, void *data)
{
	...
	if (rc)
		pr_debug("[%s] failed to deinit panel driver, rc=%d\n",
		       display->name, rc);

	rc = dsi_display_mipi_host_deinit(display);
	if (rc)
		pr_debug("[%s] failed to deinit mipi hosts, rc=%d\n",
		       display->name,
		       rc);
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc)
			pr_debug("[%s] failed to deinit phy%d driver, rc=%d\n",
			       display->name, i, rc);

		rc = dsi_ctrl_drv_deinit(display_ctrl->ctrl);
		if (rc)
			pr_debug("[%s] failed to deinit ctrl%d driver, rc=%d\n",
			       display->name, i, rc);
	}
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	if (display->is_active) {
		...
		if (rc) {
			pr_debug("device init failed, rc=%d\n", rc);
			return rc;
		}
...
int dsi_display_get_active_displays(void **display_array, u32 max_display_count)
{
	...
	list_for_each_entry(pos, &dsi_display_list, list) {
		if (i >= max_display_count) {
			pr_debug("capping display count to %d\n", i);
			break;
		}
		if (pos->is_active)
			display_array[i++] = pos;
		...
		struct drm_encoder *enc)
{
	...
	if (!priv) {
		pr_debug("Private data is not present\n");
		rc = -EINVAL;
		goto error;
	}

	if (display->bridge) {
		pr_debug("display is already initialized\n");
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(bridge)) {
		rc = PTR_ERR(bridge);
		pr_debug("[%s] brige init failed, %d\n", display->name, rc);
		goto error;
	}
		...
		struct drm_encoder *enc, struct drm_connector *connector)
{
	...
	if (!display->bridge) {
		pr_debug("dsi bridge is not initialize\n");
		rc = -EINVAL;
		goto error;
	}
...
int dsi_display_get_info(struct msm_display_info *info, void *disp)
{
	...
	if (rc) {
		pr_debug("[%s] failed to get panel phy props, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	case DSI_OP_CMD_MODE:
		...
		break;
	default:
		pr_debug("unknwown dsi panel mode %d\n",
			...
			u32 *count)
{
	...
	if (!display || !display->panel) {
		pr_debug("invalid display:%d panel:%d\n", display != NULL,
	...
	*count = display->panel->num_timing_nodes;
	...
	if (rc) {
		pr_debug("[%s] failed to get dfps caps from panel\n",
				display->name);
		return rc;
	}
			  ...
			  struct dsi_display_mode **out_modes)
{
	...
	*out_modes = NULL;
	...
	for (mode_idx = 0; mode_idx < panel_mode_count; mode_idx++) {
		...
		if (rc) {
			pr_debug("[%s] failed to get mode idx %d from panel\n",
		...
		struct dsi_display_mode **out_mode)
{
	...
	*out_mode = NULL;
	...
	if (!*out_mode) {
		pr_debug("[%s] failed to find mode for v_active %u h_active %u fps %u pclk %u\n",
			...
			struct dsi_display_mode *adj_mode)
{
	...
	if ((cur_mode->timing.v_active == adj_mode->timing.v_active) &&
		...
		if (cur_mode->pixel_clk_khz != adj_mode->pixel_clk_khz) {
			dyn_clk_caps = &(display->panel->dyn_clk_caps);
			if (!dyn_clk_caps->dyn_clk_support) {
				pr_debug("dyn clk change not supported\n");
				rc = -ENOTSUPP;
				goto error;
			}
			if (adj_mode->dsi_mode_flags & DSI_MODE_FLAG_VRR) {
				pr_debug("dfps and dyn clk not supported in same commit\n");
				rc = -ENOTSUPP;
				goto error;
			}
			      ...
			      u32 flags)
{
	...
	if (rc) {
		pr_debug("[%s] panel mode validation failed, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] ctrl mode validation failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug("[%s] phy mode validation failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
	...
	if ((flags & DSI_VALIDATE_FLAG_ALLOW_ADJUST) &&
		...
		if (rc) {
			pr_debug("[%s] seamless not possible rc=%d\n",
				display->name, rc);
			goto error;
		}
	}
			 ...
			 u32 flags)
{
	...
	if (rc) {
		pr_debug("[%s] mode cannot be set\n", display->name);
		goto error;
	}

	rc = dsi_display_set_mode_sub(display, &adj_mode, flags);
	if (rc) {
		pr_debug("[%s] failed to set mode\n", display->name);
		goto error;
	}
...
int dsi_display_set_tpg_state(struct dsi_display *display, bool enable)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("[%s] failed to set tpg state for host_%d\n",
			       display->name, i);
			goto error;
		}
	}
...
static int dsi_display_pre_switch(struct dsi_display *display)
{
	int rc = 0;
#if defined(CONFIG_DISPLAY_SAMSUNG)
	pr_debug("DMS : update dsi ctrl for new mode\n");
#endif
	...
	if (rc) {
		pr_debug("[%s] failed to update DSI controller, rc=%d\n",
			   display->name, rc);
	...
	if (rc) {
		pr_debug("[%s] failed to set DSI link clock source, rc=%d\n",
			display->name, rc);
		goto error_ctrl_deinit;
	}
...
static void dsi_display_register_error_handler(struct dsi_display *display)
{
	...
	if (!display->err_workq) {
		pr_debug("failed to create dsi workq!\n");
		return;
	}
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	if (!display->panel->cur_mode) {
		pr_debug("no valid mode set for the display");
		return -EINVAL;
	}
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		if (display->is_cont_splash_enabled) {
			pr_debug("DMS is not supposed to be set on first frame\n");
			return -EINVAL;
		}
		/* update dsi ctrl for new mode */
		rc = dsi_display_pre_switch(display);
		if (rc)
			pr_debug("[%s] panel pre-prepare-res-switch failed, rc=%d\n",
					display->name, rc);
		goto error;
	}
	...
	if (!display->is_cont_splash_enabled) {
		...
		if (rc) {
			pr_debug("[%s] panel pre-prepare failed, rc=%d\n",
					display->name, rc);
			goto error;
		}
	}
	...
	if (vdd->dtsi_data.samsung_tcon_clk_on_support) {
		LCD_INFO("increase core clk refcount to keep hs clock while display on\n");
		rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
				DSI_CORE_CLK, DSI_CLK_ON);
		if (rc) {
			pr_debug("[%s] failed to enable DSI core clocks, rc=%d\n",
	...
	if (rc) {
		pr_debug("[%s] failed to setup DSI controller, rc=%d\n",
		       display->name, rc);
		goto error_phy_disable;
	}
	...
	if (rc) {
		pr_debug("[%s] failed to enable DSI host, rc=%d\n",
		       display->name, rc);
		goto error_ctrl_deinit;
	}
...
#if defined(CONFIG_DISPLAY_SAMSUNG)
	if (vdd->dtsi_data.samsung_tcon_clk_on_support) {
		LCD_INFO("increase link clk refcount to keep hs clock while display on\n");
		rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
				DSI_LINK_CLK, DSI_CLK_ON);
		if (rc) {
			pr_debug("[%s] failed to enable DSI link clocks, rc=%d\n",
	...
	if (!display->is_cont_splash_enabled) {
		...
		if (rc) {
			pr_debug("[%s] failed soft reset, rc=%d\n",
					display->name, rc);
		...
		if (rc) {
			pr_debug("[%s] panel prepare failed, rc=%d\n",
					display->name, rc);
			goto error_ctrl_link_off;
		}
	}
		...
		struct dsi_rect *out_roi)
{
	...
	if (req_rois->num_rects > roi_caps->num_roi) {
		pr_debug("request for %d rois greater than max %d\n",
				req_rois->num_rects,
				roi_caps->num_roi);
		rc = -EINVAL;
		goto exit;
		...
		struct msm_roi_list *rois)
{
	...
	for (i = 0; i < display->ctrl_count; i++) {
		...
		if (rc) {
			pr_debug("dsi_display_calc_ctrl_roi failed rc %d\n", rc);
			return rc;
		}

		rc = dsi_ctrl_set_roi(ctrl->ctrl, &ctrl_roi, &changed);
		if (rc) {
			pr_debug("dsi_ctrl_set_roi failed rc %d\n", rc);
			return rc;
		}
		...
		if (rc) {
			pr_debug("dsi_panel_set_roi failed rc %d\n", rc);
			return rc;
		}
		...
		rc = dsi_ctrl_setup(ctrl->ctrl);
		if (rc) {
			pr_debug("dsi_ctrl_setup failed rc %d\n", rc);
			return rc;
		}
	}
...
int dsi_display_config_ctrl_for_cont_splash(struct dsi_display *display)
{
	...
	if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
		...
		if (rc) {
			pr_debug("[%s]failed to enable DSI video engine, rc=%d\n",
			       display->name, rc);
			goto error_out;
		}
		...
		rc = dsi_display_cmd_engine_enable(display);
		if (rc) {
			pr_debug("[%s]failed to enable DSI cmd engine, rc=%d\n",
			       display->name, rc);
			goto error_out;
		}
	} else {
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (display->is_cont_splash_enabled) {
		...
		if (rc) {
			pr_debug("Continuous splash res cleanup failed, rc=%d\n",
				rc);
			return -EINVAL;
		}
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		...
		if (rc) {
			pr_debug("[%s] failed to switch DSI panel mode, rc=%d\n",
				   display->name, rc);
			goto error;
		}
	} else {
		...
		if (vdd->dtsi_data.samsung_dsi_force_clock_lane_hs)
			dsi_display_continuous_clk_ctrl(display, true);
#endif
		rc = dsi_panel_enable(display->panel);
		if (rc) {
			pr_debug("[%s] failed to enable DSI panel, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
...
#endif
		if (rc) {
			pr_debug("[%s] panel pps cmd update failed, rc=%d\n",
				display->name, rc);
			goto error;
		}
	}
...
int dsi_display_post_enable(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug("[%s] panel post-enable failed, rc=%d\n",
		       display->name, rc);
...
int dsi_display_pre_disable(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug("[%s] panel pre-disable failed, rc=%d\n",
		       display->name, rc);
...
int dsi_display_disable(struct dsi_display *display)
{
	...
	rc = dsi_display_wake_up(display);
	if (rc)
		pr_debug("[%s] display wake up failed, rc=%d\n",
		       display->name, rc);

	if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
		rc = dsi_display_vid_engine_disable(display);
		if (rc)
			pr_debug("[%s]failed to disable DSI vid engine, rc=%d\n",
			       display->name, rc);
	} else if (display->config.panel_mode == DSI_OP_CMD_MODE) {
	...
	if (rc)
		pr_debug("[%s] failed to disable DSI panel, rc=%d\n",
		       display->name, rc);
...
int dsi_display_unprepare(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug("[%s] panel unprepare failed, rc=%d\n",
		       display->name, rc);

#if defined(CONFIG_DISPLAY_SAMSUNG)
	if (vdd->dtsi_data.samsung_dsi_force_clock_lane_hs)
		dsi_display_continuous_clk_ctrl(display, false);
#endif

	rc = dsi_display_ctrl_host_disable(display);
	if (rc)
		pr_debug("[%s] failed to disable DSI host, rc=%d\n",
		       display->name, rc);

	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_LINK_CLK, DSI_CLK_OFF);
	if (rc)
		pr_debug("[%s] failed to disable Link clocks, rc=%d\n",
		       display->name, rc);

#if defined(CONFIG_DISPLAY_SAMSUNG)
	if (vdd->dtsi_data.samsung_tcon_clk_on_support) {
		pr_debug("decrease link clk refcount to balance\n");
	...
	if (rc)
		pr_debug("[%s] failed to deinit controller, rc=%d\n",
		       display->name, rc);
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_CORE_CLK, DSI_CLK_OFF);
	if (rc)
		pr_debug("[%s] failed to disable DSI clocks, rc=%d\n",
		       display->name, rc);

#if defined(CONFIG_DISPLAY_SAMSUNG)
	if (vdd->dtsi_data.samsung_tcon_clk_on_support) {
		pr_debug("decrease core clk refcount to balance\n");
		rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
				DSI_CORE_CLK, DSI_CLK_OFF);
		if (rc)
			pr_debug("[%s] failed to disable DSI clocks, rc=%d\n",
	...
	if (rc)
		pr_debug("[%s] panel post-unprepare failed, rc=%d\n",
		       display->name, rc);
