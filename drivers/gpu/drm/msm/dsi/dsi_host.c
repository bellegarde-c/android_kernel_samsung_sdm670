...
static int dsi_clk_ctrl(struct msm_dsi_host *msm_host, bool enable)
{
	...
	if (enable) {
		...
		if (ret) {
			pr_debug("%s: Can not enable bus clk, %d\n",
				__func__, ret);
			goto unlock_ret;
		}
		ret = dsi_link_clk_enable(msm_host);
		if (ret) {
			pr_debug("%s: Can not enable link clk, %d\n",
				__func__, ret);
...
int msm_dsi_host_power_on(struct mipi_dsi_host *host)
{
	...
	ret = dsi_bus_clk_enable(msm_host);
	if (ret) {
		pr_debug("%s: failed to enable bus clocks, %d\n", __func__, ret);
	...
	dsi_bus_clk_disable(msm_host);
	if (ret) {
		pr_debug("%s: failed to enable phy, %d\n", __func__, ret);
		goto fail_disable_reg;
	}

	ret = dsi_clk_ctrl(msm_host, 1);
	if (ret) {
		pr_debug("%s: failed to enable clocks. ret=%d\n", __func__, ret);
		goto fail_disable_reg;
	}
