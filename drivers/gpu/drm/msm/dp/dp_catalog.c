...
#if defined(CONFIG_SEC_DISPLAYPORT) && !defined(SECDP_CALIBRATE_VXPX)
	pr_debug("aux_redrv: %u, rev_hw: %u\n",
		catalog->parser->aux_redrv, catalog->parser->rev_hw);
	if (catalog->parser->aux_redrv && catalog->parser->rev_hw >= 5) {
